...
def objective(trial):
    ...
    config = {
        ...
        'num_epochs': 15,  # Fixed for faster optimization
        'patience': 4,  # Reduced for faster optimization
        'dropout_rate': trial.suggest_float('dropout_rate', 0.1, 0.5),
        'weight_decay': trial.suggest_float('weight_decay', 0.001, 0.1, log=True),
        'warmup_ratio': trial.suggest_float('warmup_ratio', 0.05, 0.15),
        'max_length': 256,
        'max_dialogue_length': trial.suggest_int('max_dialogue_length', 8, 12),
        'hidden_size': trial.suggest_categorical('hidden_size', [128, 256, 384]),
        'num_layers': trial.suggest_int('num_layers', 1, 3),
        'max_grad_norm': trial.suggest_float('max_grad_norm', 0.5, 2.0),
        'use_focal_loss': trial.suggest_categorical('use_focal_loss', [True, False]),
        'focal_alpha': trial.suggest_float('focal_alpha', 0.5, 2.0) if trial.suggest_categorical('use_focal_loss_temp', [True, False]) else 1.0,
        'focal_gamma': trial.suggest_float('focal_gamma', 1.0, 3.0) if trial.suggest_categorical('use_focal_loss_temp2', [True, False]) else 2.0,
        'use_mixed_precision': True,
        'freeze_bert_layers': trial.suggest_int('freeze_bert_layers', 4, 8),
        'data_augmentation': trial.suggest_categorical('data_augmentation', [True, False]),
        'validation_frequency': 1,
        # Video encoder parameters (ensure embed_dim is divisible by num_heads)
        'video_embed_dim': trial.suggest_categorical('video_embed_dim', [256, 384, 512, 768]),
        'video_num_heads': trial.suggest_categorical('video_num_heads', [4, 6, 8, 12]),
        'video_num_layers': trial.suggest_int('video_num_layers', 2, 6),
        # Optimizer parameters
        'bert_lr_ratio': trial.suggest_float('bert_lr_ratio', 0.05, 0.2),
    }
    
    # Ensure video embed_dim is divisible by num_heads
    if config['video_embed_dim'] % config['video_num_heads'] != 0:
        raise optuna.TrialPruned()
    
    # Calculate effective batch size
    effective_batch_size = config['batch_size'] * config['gradient_accumulation_steps']
    if effective_batch_size > 16:  # Avoid too large batch sizes
        raise optuna.TrialPruned()
    ...
    all_labels = []
    for i in range(min(len(train_dataset), 1000)):  # Sample for faster computation
    ...
    class OptimizedTimeSformerEncoder(nn.Module):
        """Optimized TimeSformer encoder with trial parameters"""
        
        def __init__(self, num_frames=8, frame_size=224, patch_size=32, embed_dim=384, 
                     num_heads=6, num_layers=4, dropout_rate=0.1):
            super(OptimizedTimeSformerEncoder, self).__init__()
        ...
        def forward(self, x):
            ...
            for i in range(seq_len):
                ...
                for t in range(self.num_frames):
                    frame = video[:, :, t, :, :]
                    frame_patches = self.patch_embed(frame)
                    frame_patches = frame_patches.flatten(2).transpose(1, 2)
                    frame_patches = frame_patches + self.pos_embed_spatial
                    frame_features.append(frame_patches)
    ...
    class OptimizedMultimodalLSTMVideoModel(nn.Module):
        def __init__(self, dataset, config):
            super(OptimizedMultimodalLSTMVideoModel, self).__init__()
            ...
            if config['freeze_bert_layers'] > 0:
                for i, layer in enumerate(self.text_encoder.encoder.layer):
                    if i < config['freeze_bert_layers']:
                        for param in layer.parameters():
                            param.requires_grad = False
            
            # Video encoder with trial parameters
            self.video_encoder = OptimizedTimeSformerEncoder(
                num_frames=8,
                frame_size=224,
                embed_dim=config['video_embed_dim'],
                num_heads=config['video_num_heads'],
                num_layers=config['video_num_layers'],
            ...
            # LSTM modules
            text_lstm_input_size = text_dim + 16
            self.text_lstm = nn.LSTM(
                input_size=text_lstm_input_size,
            ...
            self.video_lstm = nn.LSTM(
                input_size=config['video_embed_dim'],
                hidden_size=self.hidden_size,
                num_layers=self.num_layers,
                batch_first=True,
                dropout=config['dropout_rate'] if self.num_layers > 1 else 0,
            ...
            self.metadata_temporal_processor = nn.Sequential(
                nn.Linear(metadata_dim, self.hidden_size),
            ...
            # Fusion LSTM
            fused_input_size = self.hidden_size * 2 + self.hidden_size * 2 + self.hidden_size
            self.fusion_lstm = nn.LSTM(
                input_size=fused_input_size,
                hidden_size=self.hidden_size,
                num_layers=self.num_layers,
            ...
            # Final classifier
            final_dim = 128 + self.hidden_size * 2 + 64
    ...
    model = OptimizedMultimodalLSTMVideoModel(train_dataset, config).to(device)
    ...
    if trainable_params > 150_000_000:  # 150M parameters limit
        raise optuna.TrialPruned()
    ...
    optimizer = torch.optim.AdamW([
        {'params': bert_params, 'lr': config['learning_rate'] * config['bert_lr_ratio'], 'weight_decay': config['weight_decay']},
...
def run_hyperparameter_optimization(n_trials=50, timeout=None):
    ...
    try:
        ...
        study.optimize(
            ...
            timeout=timeout,
            callbacks=[
                lambda study, trial: print(f"Trial {trial.number} finished with value: {trial.value:.4f}" if trial.value is not None else f"Trial {trial.number} was pruned")
            ]
        )
        ...
        # Save study results
        study_path = f"optuna_results/study_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
        optuna.study.save_study(study, study_path)
        
        # Save best parameters
        best_params_path = f"optuna_results/best_params_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        try:
            ...
            fig1.savefig('optuna_results/optimization_history.png')
            
            # Parameter importances
            fig2 = optuna.visualization.matplotlib.plot_param_importances(study)
            fig2.savefig('optuna_results/param_importances.png')
            
            # Parallel coordinate plot
            fig3 = optuna.visualization.matplotlib.plot_parallel_coordinate(study)
            fig3.savefig('optuna_results/parallel_coordinate.png')
            
            print(f"📊 Optimization plots saved to optuna_results/")
        ...
        print(f"Best trial so far: {study.best_trial.value:.4f} accuracy")
        return study, study.best_trial.params if study.best_trial else None
    
    except Exception as e:
        print(f"\n❌ Error during optimization: {str(e)}")
        return None, None

def train_with_config(optimized_params):
    """Train model with optimized hyperparameters"""
    
    # Merge optimized params with base config
    config = {
        'batch_size': optimized_params.get('batch_size', 6),
        ...
        'patience': optimized_params.get('patience', 5),
        'dropout_rate': optimized_params.get('dropout_rate', 0.3),
        'weight_decay': optimized_params.get('weight_decay', 0.01),
        'warmup_ratio': optimized_params.get('warmup_ratio', 0.1),
        'max_length': 256,
        'max_dialogue_length': optimized_params.get('max_dialogue_length', 10),
        'hidden_size': optimized_params.get('hidden_size', 256),
        'num_layers': optimized_params.get('num_layers', 2),
        'max_grad_norm': optimized_params.get('max_grad_norm', 1.0),
        'use_focal_loss': optimized_params.get('use_focal_loss', True),
        'focal_alpha': optimized_params.get('focal_alpha', 1.0),
        'focal_gamma': optimized_params.get('focal_gamma', 2.0),
        'use_mixed_precision': True,
        'freeze_bert_layers': optimized_params.get('freeze_bert_layers', 6),
        'data_augmentation': optimized_params.get('data_augmentation', True),
        'validation_frequency': 1,
        'video_embed_dim': optimized_params.get('video_embed_dim', 384),
        'video_num_heads': optimized_params.get('video_num_heads', 6),
        'video_num_layers': optimized_params.get('video_num_layers', 4),
        'bert_lr_ratio': optimized_params.get('bert_lr_ratio', 0.1),
    }
    ...
    # Run the original training function with optimized config
    run_regular_training(config)

def run_regular_training(config):
    """Run regular training with given configuration"""
    ...
    bert_lr_ratio = config.get('bert_lr_ratio', 0.1)
    optimizer = torch.optim.AdamW([
        {'params': bert_params, 'lr': config['learning_rate'] * bert_lr_ratio, 'weight_decay': config['weight_decay']},
        {'params': other_params, 'lr': config['learning_rate'], 'weight_decay': config['weight_decay']}
    ])
...
def main():
    """Main function with option to run hyperparameter optimization or regular training"""
    
    import sys
    
    # Check command line arguments for optimization mode
    if len(sys.argv) > 1 and sys.argv[1] == 'optimize':
        print("🔍 Running hyperparameter optimization mode...")
        ...
        if len(sys.argv) > 2:
            try:
                ...
                print("Invalid number of trials, using default: 50")
        
        if len(sys.argv) > 3:
            try:
                timeout = int(sys.argv[3])
            except ValueError:
                print("Invalid timeout, using no timeout")
        
        # Run optimization
        study, best_params = run_hyperparameter_optimization(n_trials=n_trials, timeout=timeout)
        
        if best_params:
            print(f"\n🎯 To train with best parameters, update your config with:")
            ...
            if response.lower() in ['y', 'yes']:
                ...
                train_with_config(best_params)
        
        return
    
    # Regular training mode with default configuration
    config = {
        'batch_size': 6,
    ...
    print("🚀 Running regular training mode...")
    run_regular_training(config)
    config = {
